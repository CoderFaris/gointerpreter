...
func TestGlobalLetStatetements(t *testing.T) {
	tests := []compilerTestCase{
		{
			input: `
			let one = 1;
...
func testConstants(
	...
	if len(expected) != len(actual) {
		return fmt.Errorf("wrong number of constants. want=%q, got=%q", len(expected), len(actual))
	}
	...
	for i, constant := range expected {
		...
		case string:
			...
			if err != nil {
				return fmt.Errorf("constant %d - testString object failed: %s", i, err)
			}
